...
clear all
%close all
addpath("/Users/katietaylor/Library/Mobile Documents/com~apple~CloudDocs/Research/PIV/Solar/Solar_Functions")

project_path   = '/Users/katietaylor/Library/Mobile Documents/com~apple~CloudDocs/Research/PIV/Solar/Results/';
inpt_name      = 'D_SS_NOM_S15_H3_A045_B225_PIVMID_UH_REMAP';
mean_file      = strcat(project_path, 'means'  , '/', inpt_name, '_MEANS.mat');
...
dyz=abs(y(2)-y(1)); %grid spacing in y and z

x=0:dyz:170*dyz; %building the x coodrinate 
...
thld_V = min(V) + range(V)*0.001;
thld_W = min(W) + range(W)*0.001;
acc_V = V >= thld_V;
acc_W = W >= thld_W;
...
[ZZZ,YYY,XXX]=meshgrid(z,y,x); %Building the 3D mesh


% Normal Turbuluent Stresses
uu = means.uu./U_inf_sq;
vv = means.vv./U_inf_sq;
ww = means.ww./U_inf_sq;

% Shear turbulent Stresses
uv = means.uv./U_inf_sq;
uw = means.uw./U_inf_sq;
vw = means.vw./U_inf_sq;

% Normal Dispersive stresses
uud=means.uud./U_inf_sq;
vvd=means.vvd./U_inf_sq;
wwd=-means.wwd./U_inf_sq;

% Shear Dispersive stresses
uvd=means.uvd./U_inf_sq;
uwd=-means.uwd./U_inf_sq;
vwd=means.vwd./U_inf_sq;
...
% for i=1:length(x)
% 
%     Ud3d(:,i:end,i)=Udnew(:,1:end-(i-1));
%     Ud3d(:,1:i-1,i)=Udnew(:,end-(i-1)+1:end);
% 
%     Vd3d(:,i:end,i)=Vdnew(:,1:end-(i-1));
%     Vd3d(:,1:i-1,i)=Vdnew(:,end-(i-1)+1:end);
% 
%     Wd3d(:,i:end,i)=Wdnew(:,1:end-(i-1));
%     Wd3d(:,1:i-1,i)=Wdnew(:,end-(i-1)+1:end);
% 
%     % Vord3d(:,i:end,i)=vord(:,1:end-(i-1));
%     % Vord3d(:,1:i-1,i)=vord(:,end-(i-1)+1:end);
% 
% end

for i = 1:length(x)
    % First part: Take data from the first i columns of Udnew, Vdnew, Wdnew
    Ud3d(:, 1:i, i) = Udnew(:, end-(i-1):end); 
    Vd3d(:, 1:i, i) = Vdnew(:, end-(i-1):end); 
    Wd3d(:, 1:i, i) = Wdnew(:, end-(i-1):end); 
    
    % Second part: Take data from columns i+1 to the end of Udnew, Vdnew, Wdnew
    Ud3d(:, i+1:end, i) = Udnew(:, 1:end-(i));
    Vd3d(:, i+1:end, i) = Vdnew(:, 1:end-(i)); 
    Wd3d(:, i+1:end, i) = Wdnew(:, 1:end-(i)); 
end
...
% clim([-l_Du l_Du]);
quiver(ZZrot(1:vn:end,1:vn:end),YYrot(1:vn:end,1:vn:end),Wdrot(1:vn:end,1:vn:end),Vdrot(1:vn:end,1:vn:end),2*Scale,'color',[0 0 0]);
